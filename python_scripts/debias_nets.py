...
def data_setup(config):
    ...
    prep_dict["test2"] = prep.image_test(**config["prep"]['params_target'])
    ...
    dset_loaders["test"] = DataLoader(dsets["test"], batch_size=test_bs, \
                                      shuffle=False, num_workers=4)
    dsets["test2"] = ImageList(open(data_config["test2"]["list_path"]).readlines(),
                              transform=prep_dict["test2"], labelled=data_config["test"]["labelled"])
    dset_loaders["test2"] = DataLoader(dsets["test2"], batch_size=test_bs, \
                                      shuffle=False, num_workers=4)
...
def train(config, dset_loaders):
    ...
    for itr in range(config["num_iterations"]):
        if itr % config["snapshot_interval"] == 0:
            ...
            # obtain_lab(dset_loaders['test'],temp_model)
        ...
        if itr % config["test_interval"] == 0:
            ...
            test_info = validation_loss(dset_loaders, base_network, data_name='test', dset=config['dataset'],
                                        num_classes=config["network"]["params"]["class_num"],
                                        logs_path=config['logs_path'], is_training=config['is_training'])

            test2_info = validation_loss(dset_loaders, base_network, data_name='test2', dset=config['dataset'],
                                        num_classes=config["network"]["params"]["class_num"],
                                        logs_path=config['logs_path'], is_training=config['is_training'])
            ...
            print_msg("Iteration: " + str(itr) + "/"+ str(config["num_iterations"])+ " | Val loss: "+ str(val_info['val_loss'])+
                  " | Val Accuracy: "+ str(val_info['val_accuracy'])
                    ,config["out_file"])
            print_msg("Iteration: " + str(itr) + "/"+ str(config["num_iterations"])+ " | test loss: "+ str(test_info['val_loss'])+
                  " | test Accuracy: "+ str(test_info['val_accuracy'])
                    ,config["out_file"])
            print_msg("Iteration: " + str(itr) + "/" + str(config["num_iterations"]) + " | test2 loss: " + str(
                test2_info['val_loss']) +
                      " | test Accuracy: " + str(test2_info['val_accuracy'])
                      , config["out_file"])
            temp_model = nn.Sequential(base_network)
        ...
        inputs_target, labels_target,_,_ = iter_target.next()
...
def obtain_lab(dset_loader, model):
    centers = 2
    l, n, m, o = obtain_label(dset_loader, model=model, num_classes=centers)
...
def test(config, dset_loaders, model_path_for_testing=None):
        ...
        print_msg("Final Model " + "| Val loss: " + str(val_info['val_loss']) + str("| Val Accuracy: ") + str(
            val_info['val_accuracy']) + (
                      "| 2 class acc:" + str(val_info['val_acc_2_class']) if 'val_acc_2_class' in val_info else ""),
                  config["out_file"])
...
def parge_args():
    ...
    parser.set_defaults(
        ...
        gpu_id="0",
        dset="embryo",
        s_dset_txt="../../data/embryo/_source.txt",
        sv_dset_txt="../../data/embryo/_all_val.txt",
        test_dset_txt="../../data/embryo/_all.txt",
        t_dset_txt="../../data/embryo/_target.txt",
        test2_dset_txt="../../data/embryo/_ed1_test.txt",
        s_dset="ED4_001",
        t_dset="ED1",
        lr=0.01,
        arch="Xception",
...
def main():
    ...
    test2_input = {'path': args.test2_dset_txt, 'labelled': args.target_labelled.lower() == 'true'}
    ...
    if not is_training:
        model_path_for_testing = models_root + args.dset + '/' + "train_"+args.s_dset + "_to_" + args.t_dset + "/best_model.pth.tar"
    ...
    config["data"] = {"source": {"list_path": source_input['path'], "batch_size": args.batch_size},
                      ...
                      "test2": {"list_path": test2_input['path'], "batch_size": args.batch_size_test,
                               "labelled": test2_input['labelled']},
    ...
    if is_training:
        ...
        test(config, dset_loaders, model_path_for_testing=models_root + args.dset+ '/' +  args.arch + '/' + "train_"+args.s_dset + "_to_" + args.t_dset + "/best_model.pth.tar")
