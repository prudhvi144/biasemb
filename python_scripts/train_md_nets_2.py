...
def data_setup(config):
    ...
    dsets["source_base"] = ImageList(open(data_config["source_base"]["list_path"]).readlines(),
                                transform=prep_dict["source"])
    dset_loaders["source_base"] = DataLoader(dsets["source_base"], batch_size=train_bs, shuffle=False, num_workers=4,
                                        drop_last=True)
    dsets["source"] = ImageList(open(data_config["source"]["list_path"]).readlines(),
                                transform=prep_dict["source"])
    dset_loaders["source"] = DataLoader(dsets["source"], batch_size=train_bs, shuffle=False, num_workers=4, drop_last=True)
...
def test(config, dset_loaders, model_path_for_testing=None):
    ...
    test_info = validation_loss(dset_loaders, model, data_name='test',dset=config['dataset'],
                                ...
                                logs_path=config['logs_path'], is_training=config['is_training'])
    centers = 1
    l, n , m ,o = obtain_label(dset_loaders['test'], model=model, num_classes=centers)
...
def parge_args():
    ...
    parser.set_defaults(
        ...
        sv_dset_txt="../../data/embryo/_source.txt",
        t_dset_txt="../../data/embryo/_target.txt",
        sb_dset_txt="../../data/embryo/_train.txt",
        s_dset="d",
        t_dset="a",
        lr=0.001,
        arch="ResNet50",
        ...
        image_size=256,
        crop_size=224,
        trained_model_path= "../../models/embryo1/ResNet50/train_d_to_a/best_model.pth.tar",
...
def main():
    ...
    source_base = {'path': args.sb_dset_txt}
    ...
    config["data"] = {"source_base": {"list_path": source_base['path'], "batch_size": args.batch_size},
        "source": {"list_path": source_input['path'], "batch_size": args.batch_size},
    ...
    if is_training:
        ...
        print("=" * 50)
        print()
        train_base(config, dset_loaders)
        print()
        print("=" * 50)
