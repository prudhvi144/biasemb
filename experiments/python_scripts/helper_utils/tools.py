...
def obtain_label(loader, model,num_classes):
    ...
    kmeans = KMeans(2, random_state=0).fit(all_fea)
    ...
    plt.scatter(all_fea[:, 0], all_fea[:, 1], c=all_lab, s=50, cmap='viridis')

    centers = kmeans.cluster_centers_
    plt.scatter(centers[:, 0], centers[:, 1], c='black', s=200, alpha=0.5)
    plt.show()
    plt.scatter(all_fea[:, 0], all_fea[:, 1], c=labels, s=50, cmap='viridis')
